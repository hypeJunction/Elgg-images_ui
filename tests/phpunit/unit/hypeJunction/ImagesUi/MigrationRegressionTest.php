...
namespace hypeJunction\ImagesUi\Tests;

use PHPUnit\Framework\TestCase;

/**
 * Migration regression guard for plugin "images_ui" (Elgg 7.x target).
 *
 * Derived from skills/elgg-test-writer/templates/MigrationRegressionTest.php.template
 ...
 * ABSENT for the target major (Elgg 7.x). Static (no Elgg boot) because most
 * catalog classes fatal at class-load or page-render on the target version, so
 * the signature has to be caught in the source.
 *
 * NOTE (plugin-specific): this plugin ships a git-ignored `mod/` harness that
 * checks out sibling plugins (images, proxy) for local Docker testing, plus a
 * `docker/` dir. Those are NOT part of images_ui's own source, so the file
 * scanner below skips them in addition to the usual vendor dirs — otherwise the
 * guard would flag another plugin's pre-7.x code.
 *
 * Catalog classes covered (target-gated to 7.x):
 *   FC-*x*x removed symbols ...... testNoRemovedSymbols
 *   FC-6x7x-01 removed constant .. testNoRemovedConstants
 *   FC-3x4x-04 detectMimeType .... testNoRemovedStaticMimeType
 *   FC-3x4x-01 forbidden files ... testNoForbiddenBootstrapFiles
 *   FC-3x4x-10 lowercase id ...... testNoCamelCasePluginIdCallsites
 *   FC-5x6x-01/04 class contracts  testNoRemovedClassContracts
 *   FC-5x6x-03 Seed shape ........ testSeedSubclassShape
 *   FC-5x6x-04 Batch shape ....... testUpgradeBatchShape
...
final class MigrationRegressionTest extends TestCase
{
    ...
    /** @return list<string> files with $ext under $sub (relative dir); vendor + harness dirs skipped */
    private static function files(string $sub, string $ext): array
    {
        ...
        foreach ($it as $f) {
            $path = $f->getPathname();
            // Skip Composer/npm vendor trees AND this plugin's git-ignored
            // Docker test harness (mod/ = sibling plugins, docker/ = infra).
            if (preg_match('#/(vendor|vendors|bower_components|node_modules|mod|docker)/#', $path)) {
                continue;
            }
    ...
    public function testNoRemovedStaticMimeType(): void
    {
        ...
        foreach (self::files('', 'php') as $file) {
            ...
            if (preg_match('/(ElggFile::|->)detectMimeType\s*\(/', (string) file_get_contents($file))) {
                $violations[] = basename($file) . ': detectMimeType() removed in 4.x — use getMimeType() / mime_content_type($path)';
            }
        }
    ...
    public function testNoRouteRewriteAtInit(): void
    {
        ...
        $this->assertNotSame('', $src, 'elgg-plugin.php must exist');
    ...
    public function testMenuRegisterReturnsArray(): void
    {
        ...
        // fatals. images_ui's menu handler lives in lib/functions.php, not classes/,
        // so scan the whole plugin source (excluding tests + harness).
        $violations = [];
        foreach (self::files('', 'php') as $file) {
            if (str_contains($file, '/tests/')) {
                continue;
            ...
            $src = (string) file_get_contents($file);
            if (!preg_match('/menu:entity|MenuItems|ElggMenuItem|setup_entity_menu/', $src)) {
                continue;
            }
